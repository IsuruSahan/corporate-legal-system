<?php
// CRITICAL: Ensure this file is saved with "UTF-8 without BOM" encoding.
// There must be absolutely no spaces, empty lines, or characters above the <?php tag.

// 1. Force output buffering immediately to capture any unexpected inclusion whitespace
ob_start();

// 2. Safe session context wrapper initialization
...
// 3. Completely clear out any buffered spaces or noise generated by the required files
if (ob_get_length()) {
    ob_clean();
}

// 4. Set the JSON header safely now that the buffer is verified clean
...
// Viewer users may only call safe read endpoints (Added payment retrieval and dashboard metric filtering)
$readOnlyActions = [
...
if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'Viewer' && !in_array($action, $readOnlyActions)) {
...
if ($table === 'court_cases') {
            ...
            if ($caseStatus)   $query .= " AND LOWER(TRIM(cc.case_status)) = LOWER(TRIM(:case_status))";
            if ($searchTerm)   $query .= " AND (cc.case_parties LIKE :search OR cc.case_description LIKE :search OR cc.case_number LIKE :search)";
        ...
        elseif ($table === 'court_cases') {
            ...
            if ($caseStatus)   $stmt->bindValue(':case_status', trim($caseStatus), PDO::PARAM_STR);
            if ($searchTerm)   $stmt->bindValue(':search', '%' . $searchTerm . '%', PDO::PARAM_STR);
        }
        elseif ($table === 'payments') {
            if ($companyId)    $stmt->bindValue(':company_id', $companyId, PDO::PARAM_INT);
            ...
            <div style="margin-top: 24px;">
                ...
                <div class="feed-list" style="display: flex; flex-direction: column; gap: 14px;">
                    <?php foreach ($activeLitigations as $lit): ?>
                        <a href="<?php echo BASE_URL; ?>court-cases/index.php" style="text-decoration: none; display: flex; justify-content: space-between; align-items: center; padding: 6px 0;">
